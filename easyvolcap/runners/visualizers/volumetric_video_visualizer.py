...
class VolumetricVideoVisualizer:  # this should act as a base class for other types of visualizations (need diff dataset)
    def __init__(self,
                 ...
                 store_alpha_channel: bool = False,  # store rendered acc in alpha channel #TBD True
    ...
    def generate_type(self, output: dotdict, batch: dotdict, type: Visualization = Visualization.RENDER):
        ...
        elif type == Visualization.REFRACTED:
            if 'refracted_rgb_map' not in output: return None, None, None
            img = output.refracted_rgb_map
        
        elif type == Visualization.REFRACTION:
            if 'refraction_map' not in output: return None, None, None
            img = output.refraction_map.expand(output.refraction_map.shape[:-1] + (3,))


        # elif type == Visualization.PLANE:
        #     if 'plane_dpt_map' not in output: return None, None, None
        #     if self.dpt_curve == 'linear':
        #         img = output.plane_dpt_map
        #     else:
        #         img = depth_curve_fn(output.plane_dpt_map, cm=self.dpt_cm)
        elif type == Visualization.PLANE:
            if 'ref_plane_rgb_map' not in output: return None, None, None
            img = output.ref_plane_rgb_map
        else:
