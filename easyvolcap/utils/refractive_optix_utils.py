...
from easyvolcap.utils.refractive_gaussian2d_utils import GaussianModel, dpt2norm
    ...
    def render_gaussians(self,
        ...
        if pcd.render_reflection:
            ...
            if pcd.get_specular.shape[0] != means3D.shape[0]:
                others_precomp = torch.cat([
                    torch.ones_like(opacities) * pcd.init_specular, torch.ones_like(opacities) * pcd.init_refraction], 
                dim=-1).contiguous()  # (P, 2)
            # For reflection rendering, we need to render specular and (refraction, no use for now)
            else:
                others_precomp = torch.cat([pcd.get_specular, pcd.get_refraction], dim=-1).contiguous()  # (P, 2)
        ...
        if pcd.render_reflection:
            output.update(dotdict(
                ...
                refraction=aux[..., pcd.specular_channels:pcd.specular_channels+1].permute(2, 0, 1))  # (1, H, W)
            )
