...
@torch.jit.script
def getView2World(R: torch.Tensor, t: torch.Tensor) -> torch.Tensor: #TBD
    """
    T = [ R, t;
          0, 1 ]
     T⁻¹ = [ Rᵀ, - Rᵀ t; 0, 1 ]
    """
    Rt = R.transpose(-2, -1)          # Rᵀ
    t_inv = -torch.matmul(Rt, t)        # -Rᵀ t
...
def prepare_gaussian_camera(batch):
    ...
    #TBD
    output.view_world_transform = getView2World(R, T).transpose(0, 1) #camera coordinates to world coordinates
...
@torch.jit.script
def refraction_activation(x):
    return torch.sigmoid(x)


@torch.jit.script
def inverse_refraction_activation(x):
    return torch.logit(torch.clamp(x, 1e-6, 1 - 1e-6))
...
def get_step_lr_func(lr_init, lr_steps, lr_decay_factor=0.1, lr_final=None, start_steps=0):
    """Step-based learning rate scheduler with minimum lr and start steps"""
    def step_scheduler(step):
        # start_steps 이전에는 0 학습률
        if step < start_steps:
            return 0.0
            
        # start_steps부터 시작하도록 조정
        adjusted_step = step - start_steps
        
        lr = lr_init
        for decay_step in lr_steps:
            if adjusted_step >= decay_step:
                lr *= lr_decay_factor
            else:
                break
        
        # lr_final보다 낮아지지 않도록 제한
...
def get_expon_lr_func(
    ...
    def helper(step):
        if step < start_steps or (lr_init == 0.0 and lr_final == 0.0):
            # 시작 전에는 0 학습률 또는 비활성화
            return 0.0
        ...
        t = np.clip(adjusted_step / max_steps, 0, 1)
...
class GaussianModel(nn.Module):
    def __init__(
        ...
        refraction_channels: int = 1,
        init_specular: float = 1e-3,
        init_refraction: float = 0.5, #TBD 1e-3
        max_gs: int = 1e6,
        ...
        self.create_from_pcd(xyz, colors, init_occ, init_scale, specular_channels, init_specular, init_refraction)
        self.render_reflection = render_reflection
        self.specular_channels = specular_channels
        self.refraction_channels = refraction_channels
        self.init_specular = init_specular
        self.init_refraction = init_refraction
    ...
    def setup_functions(
        ...
        refraction_activation=torch.sigmoid,
        refraction_inverse_activation=torch.logit
    ):
        ...
        self.refraction_activation = getattr(torch, refraction_activation) if isinstance(refraction_activation, str) else refraction_activation
        self.refraction_inverse_activation = getattr(torch, refraction_inverse_activation) if isinstance(refraction_inverse_activation, str) else refraction_inverse_activation
    ...
    def get_refraction(self):
        return self.refraction_activation(self._refraction)
    ...
    def create_from_pcd(
        ...
        if not isinstance(refraction, torch.Tensor) or len(refraction) != len(xyz):
            refraction = refraction * torch.ones((xyz.shape[0], 1), dtype=torch.float)
        refraction = self.refraction_inverse_activation(refraction)
        self._specular = make_params(specular)
        self._refraction = make_params(refraction)
    ...
    def distort_color_refraction(self, range: float = 0.4, threshold: float = 0.9, optimizer: Optimizer = None, prefix: str = ''): #TBD
        log(yellow_slim(f'[DISTORT COLOR using REFRACTION]'))
        mask = self.get_refraction.max(dim=-1).values.flatten() > threshold  # (P,)
        features_dc = self._features_dc.clone()
        new_features_dc = features_dc + torch.rand_like(features_dc) * range * 2 - range
        new_features_dc[mask] = features_dc[mask]
        new_features_dc.grad = self._features_dc.grad
    ...
    def zero_out_color_black(self, threshold: float = 0.9, optimizer: Optimizer = None, prefix: str = ''): #TBD Reset color to black for RGB [0, 0, 0] using refraction
        log(yellow_slim(f'[ZERO OUT COLOR BLACK using REFRACTION]'))
        mask = self.get_refraction.max(dim=-1).values.flatten() > threshold
        features_dc = self._features_dc.clone()
        new_features_dc = features_dc.clone()
        # RGB [0, 0, 0]에 해당하는 SH 계수
        # sh02rgb: rgb = sh * C0 + 0.5
        # 0 = sh * C0 + 0.5 → sh = -0.5 / C0
        C0 = 0.28209479177387814
        black_sh = torch.full_like(features_dc[mask], -0.5 / C0)
        new_features_dc[mask] = black_sh
        new_features_dc.grad = self._features_dc.grad
    ...
    def enlarge_refraction(self, enlarge_refraction: float = 0.9, optimizer: Optimizer = None, prefix: str = ''):
        log(yellow_slim(f'[ENLARGE REFRACTION] ENLARGE REACTION TO {enlarge_refraction}'))
        new_refraction = torch.max(self._refraction, self.refraction_inverse_activation(torch.ones_like(self._refraction, ) * enlarge_refraction))
        new_refraction.grad = self._refraction.grad
        self._refraction = self.replace_tensor_to_optimizer(new_refraction, '_refraction', optimizer, prefix)
    ...
    def reset_refraction(self, reset_refraction: float = 0.001, optimizer: Optimizer = None, prefix: str = '', reset_refraction_all: bool = False): #TBD
        log(yellow_slim(f'[RESET refraction] RESET refraction TO {reset_refraction}'))
        if reset_refraction_all: new_refraction = self.refraction_inverse_activation(torch.ones_like(self._refraction, ) * reset_refraction)
        else: new_refraction = torch.min(self._refraction, self.refraction_inverse_activation(torch.ones_like(self._refraction, ) * reset_refraction))
        new_refraction.grad = self._refraction.grad
        self._refraction = self.replace_tensor_to_optimizer(new_refraction, '_refraction', optimizer, prefix)
    ...
    def densification_postfix(
        ...
        new_refraction: torch.Tensor = None
    ):
        d = dotdict({
            "_xyz": new_xyz,
    ...
    def clone(self, mask, optimizer: Optimizer = None, prefix: str = ''):
        ...
        new_refraction = self._refraction[mask]
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, optimizer, prefix, new_specular, new_refraction)
    ...
    def split(self, mask, N: int = 2, optimizer: Optimizer = None, prefix: str = '', ratio: float = 0.8):
        ...
        new_refraction = self._refraction[mask].repeat(N, 1)
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, optimizer, prefix, new_specular, new_refraction)
    ...
    @torch.no_grad()
    def set_refraction_for_upper_gaussians_by_position(self, plane_normal, batch: dotdict, optimizer: Optimizer = None, prefix: str = ''):
        """
        카메라보다 실제로 높은 위치에 있는 gaussian들의 refraction을 1.0으로 설정
        """
        from easyvolcap.utils.math_utils import normalize
        
        # batch에서 직접 카메라 정보 추출
        R, T = batch.R[0], batch.T[0]  # (3, 3), (3, 1)
        camera_center = (-R.mT @ T)[..., 0]  # (3,) - 카메라 월드 좌표
        
        up_direction = normalize(plane_normal)  # (3,)
        gaussian_positions = self.get_xyz  # (N, 3)
        cam_to_gaussian = gaussian_positions - camera_center.unsqueeze(0)  # (N, 3)
        height_projection = torch.sum(cam_to_gaussian * up_direction, dim=-1)  # (N,)
        upper_mask = height_projection > 0  # (N,)
        
        if upper_mask.any():
            new_refraction = self._refraction.clone()
            target_refraction = self.refraction_inverse_activation(torch.ones_like(new_refraction[upper_mask]) * 1.0)
            new_refraction[upper_mask] = target_refraction
            new_refraction.grad = self._refraction.grad
            self._refraction = self.replace_tensor_to_optimizer(new_refraction, "_refraction", optimizer, prefix)
            
            num_updated = upper_mask.sum().item()
            log(yellow_slim(f'[REFRACTION UPPER POS] Set refraction=1.0 for {num_updated} gaussians (above camera)'))

    @torch.no_grad()
    def set_refraction_for_upper_gaussians_by_plane(self, plane_normal: torch.Tensor, 
        ...
        gaussian_heights = torch.sum(xyz * up_direction, dim=-1)  # (N,)
        plane_height = plane_offset.item() if torch.is_tensor(plane_offset) else plane_offset
        
        # 평면보다 위에 있는 가우시안들 찾기
        upper_gaussians = gaussian_heights > plane_height  # (N,)
        
        if upper_gaussians.sum() > 0:
            # refraction을 1.0으로 설정
            new_refraction = self._refraction.clone()
            target_refraction = torch.ones_like(new_refraction[upper_gaussians]) * 1.0
            new_refraction[upper_gaussians] = self.refraction_inverse_activation(target_refraction)
            new_refraction.grad = self._refraction.grad
            self._refraction = self.replace_tensor_to_optimizer(new_refraction, '_refraction', optimizer, prefix)
            
            log(blue_slim(f'[SET REFRACTION BY PLANE] Set {upper_gaussians.sum()} upper gaussians refraction to 1.0'))
            log(blue_slim(f'[PLANE INFO] Plane height: {plane_height:.4f}, '
                        f'Gaussians above plane: {upper_gaussians.sum()}/{len(gaussian_heights)}'))
        else:
            log(blue_slim(f'[SET REFRACTION BY PLANE] No gaussians found above plane at height {plane_height:.4f}'))


    @torch.no_grad()
    def reset_refraction_for_lower_gaussians_by_plane(self, plane_normal: torch.Tensor, 
                                                    plane_offset: torch.Tensor,
                                                    init_refraction: float = 0.5,
                                                    optimizer: Optimizer = None, 
                                                    prefix: str = ''):
        from easyvolcap.utils.math_utils import normalize
        ...
        current_refraction = self.get_refraction.squeeze(-1)  # (N,)
        
        below_plane = gaussian_heights <= plane_height  # (N,)
        high_refraction = current_refraction > init_refraction  # (N,)
        reset_gaussians = below_plane & high_refraction  # (N,)
        
        if reset_gaussians.sum() > 0:
            # refraction을 init_refraction으로 설정
            new_refraction = self._refraction.clone()
            target_refraction = torch.ones_like(new_refraction[reset_gaussians]) * init_refraction
            new_refraction[reset_gaussians] = self.refraction_inverse_activation(target_refraction)
            new_refraction.grad = self._refraction.grad
            self._refraction = self.replace_tensor_to_optimizer(new_refraction, '_refraction', optimizer, prefix)
            
            log(blue_slim(f'[RESET REFRACTION BY PLANE] Reset {reset_gaussians.sum()} lower gaussians refraction to {init_refraction}'))
            log(blue_slim(f'[PLANE INFO] Plane height: {plane_height:.4f}, '
                        f'Total below plane: {below_plane.sum()}, '
                        f'High refraction: {high_refraction.sum()}, '
                        f'Reset candidates: {reset_gaussians.sum()}/{len(gaussian_heights)}'))

    @torch.no_grad()
    def zero_opacity_above_plane(self, plane_normal: torch.Tensor, 
                                plane_offset: torch.Tensor,
                                optimizer: Optimizer = None, 
                                prefix: str = ''):
        """
        평면보다 위에 있는 가우시안들의 opacity를 0으로 설정
        """
        from easyvolcap.utils.math_utils import normalize
        ...
        # 평면보다 위에 있는 가우시안들 찾기
        above_plane = gaussian_heights > plane_height  # (N,)
        
        if above_plane.sum() > 0:
            num_above = above_plane.sum().item()
            
            # opacity를 0으로 설정
            new_opacity = self._opacity.clone()
            new_opacity[above_plane] = self.opacity_inverse_activation(torch.zeros_like(new_opacity[above_plane]))
            new_opacity.grad = self._opacity.grad
            self._opacity = self.replace_tensor_to_optimizer(new_opacity, '_opacity', optimizer, prefix)
            
            log(red_slim(f'[ZERO OPACITY ABOVE PLANE] Set {num_above} gaussians opacity to 0 above plane at height {plane_height:.4f}'))
        else:
            log(red_slim(f'[ZERO OPACITY ABOVE PLANE] No gaussians found above plane at height {plane_height:.4f}'))


    @torch.no_grad()
    def zero_opacity_above_camera(self, plane_normal: torch.Tensor, 
                                batch: dotdict,
                                optimizer: Optimizer = None, 
                                prefix: str = ''):
        """
        카메라보다 위에 있는 가우시안들의 opacity를 0으로 설정
        """
        from easyvolcap.utils.math_utils import normalize
        
        up_direction = normalize(plane_normal)
        R, T = batch.R[0], batch.T[0]
        camera_center = (-R.mT @ T)[..., 0]
        camera_height = torch.sum(camera_center * up_direction)
        
        xyz = self.get_xyz
        gaussian_heights = torch.sum(xyz * up_direction, dim=-1)
        
        # 카메라보다 위에 있는 가우시안들 찾기
        above_camera = gaussian_heights > camera_height
        
        if above_camera.sum() > 0:
            num_above = above_camera.sum().item()
            
            # opacity를 0으로 설정
            new_opacity = self._opacity.clone()
            new_opacity[above_camera] = self.opacity_inverse_activation(torch.zeros_like(new_opacity[above_camera]))
            new_opacity.grad = self._opacity.grad
            self._opacity = self.replace_tensor_to_optimizer(new_opacity, '_opacity', optimizer, prefix)
            
            log(red_slim(f'[ZERO OPACITY ABOVE CAMERA] Set {num_above} gaussians opacity to 0 above camera at height {camera_height:.4f}'))
        else:
            log(red_slim(f'[ZERO OPACITY ABOVE CAMERA] No gaussians found above camera at height {camera_height:.4f}'))
...
def render(
    ...
    if pc.render_reflection and colors_precomp is not None:
        colors_precomp = torch.cat([colors_precomp, pc.get_specular, pc.get_refraction], dim=-1)  # (P, C+2)
    ...
    if pc.render_reflection:
        output.update(dotdict(
            ...
            refraction=rendered_image[3+pc.specular_channels:3+pc.specular_channels+1]  # (1, H, W)
        ))
    ...
    #render_alpha = render_alpha/render_alpha #TBD just test
